...
func GetBaseAccessToken(config *service.Config) (result maps.Param, err error) {
	...
	var url = fmt.Sprintf("https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%s&secret=%s", config.AppId, config.Secret)
...
func GetOpenId(config *service.Config, jsCode string) (result maps.Param, err error) {
	...
	var url = fmt.Sprintf("https://api.weixin.qq.com/sns/jscode2session?appid=%s&secret=%s&js_code=%s&grant_type=authorization_code", config.AppId, config.Secret, jsCode)
	response, err := service.Request(config, "GET", url, vars.ContentTypeJSON, nil)
	if err != nil {
		return nil, err
	}
...
func CheckEncryptData(accessToken, encryptedMsgHash string) (valid bool, err error) {
	...
	bytesData, _ := service.Marshal(param)
	response, err := service.Request(service.NewConfig(), "POST", url, vars.ContentTypeJSON, bytes.NewReader(bytesData))
	if err != nil {
		return false, err
	}
	defer response.Body.Close()
...
func DecryptOpenData(config *service.Config, code, encryptedData, ivStr string) (result maps.Param, err error) {
	...
	plainData, err := aess.DecryptAES128CBCPKCS7(config.Cipher, encryptedData, sessionKey, iv)
	if err != nil {
		return
	}
